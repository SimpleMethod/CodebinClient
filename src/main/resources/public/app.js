        ...
        function (response) {
            ...
            if (!response.data.container_create) {
                if ($scope.supportedPlatforms.indexOf(response.data.language) !== -1) {
                    ...
                    $http({
                        url: 'https://145.239.31.229/github/local',
                        method: 'GET'
                    }).then(
                        function (local) {
                            ...
                            $http({
                                url: 'https://145.239.31.229/srv/container/create',
                                method: 'POST',
                                params: {
...
app.controller('ContainersController', ['$filter', '$routeParams', '$scope', '$http', function ($filter, $routeParams, $scope, $http, $window) {
    $http({
        url: 'https://145.239.31.229/srv/user/container/info',
        method: 'GET',
        ...
        function (response) {
            ...
            $scope.dockerRestart = function () {
                $http({
                    url: 'https://145.239.31.229/srv/container/' + $scope.containersId + '/restart',
                    method: 'POST'
                }).then(
                    function (response) {
                    },
            ...
            $scope.dockerRemove = function () {
                $http({
                    url: 'https://145.239.31.229/srv/container/' + $scope.containersId + '/delete',
                    method: 'DELETE'
                }).then(
                    function () {
            ...
            $scope.execCommands = function (path, args) {
                $http({
                    url: 'https://145.239.31.229/srv/container/' + $scope.containersId + '/exec',
                    method: 'POST',
                    params: {
                        path: path,
                        argument: args
            ...
            $http({
                url: 'https://145.239.31.229/srv/container/' + $scope.containersId + '/logs',
                method: 'GET'
            }).then(
                function (logGet) {
            ...
            $http({
                url: 'https://145.239.31.229/srv/container/' + $scope.containersId + '/top',
                method: 'GET'
            }).then(
                function (response) {
...
app.controller('HomeController', function ($scope, $http, $cookies) {
    ...
    $http({
        url: 'https://145.239.31.229/github/user',
        method: 'GET'
    }).then(
        function (response) {
...
app.controller('ProjectsController', function ($scope, $http) {
    ...
    $http({
        url: 'https://145.239.31.229/github/user/repos/public',
        method: 'GET'
    }).then(
        function (response) {
...
app.controller('CheckLoginStatus', function ($scope, $http, $cookies) {
    $scope.lastVal = $cookies.get('token');
    $http({
        url: 'https://145.239.31.229/github/user/checktoken',
        method: 'GET',
...
app.controller('ProfileController', function ($scope, $http) {
    $http.get('https://145.239.31.229/github/user/subscription').then(
        function (response) {
